/*
 * $Id: utils.c,v 1.1 2014/04/15 15:34:47 xzm Exp $
 * 
...
FILE *my_tmpfile(char *_template, const char *path)
{
	char i;
	char *p = _template;
	char *filename;
	...
	filename = malloc(strlen(_template) + strlen(path) + 2);
	if (filename == NULL)
		return NULL;
	...
	do {
		...
		sprintf(filename, "%s/%s", path, _template);
