...
main(int argc, char* argv[])
{
    ...
    string maddr = default_mgroup_addr;
    arg.findLast("--maddr=", maddr);
    string serv = default_mgroup_port;
    arg.findLast("--mport=", serv);
    CMultiCast mc(maddr, serv);

    if (arg.found("--recv"))
    {
        string ifname;
        if (arg.findLast("--ifname=", ifname))
            mc.set_join_if(ifname.c_str(), 0);
    ...
    else if (arg.found("--send"))
    {
        int max_speed;
        string strNum;
        if (arg.findLast("--max-speed=", strNum))
            max_speed = stoi(strNum);
        else
            max_speed = -1;
